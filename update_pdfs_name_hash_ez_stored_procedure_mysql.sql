...
CREATE PROCEDURE updatenamehash() 
  ...
  `ezdbfile_data_old_name_hash` varchar(250) COLLATE utf8_bin DEFAULT 'application/octet-stream'
	...
	update ezdbfile
	set name = replace(name,'var/oldpath', 'var/newPath')
	WHERE scope = 'binaryfile';

	update ezdbfile
	set name_trunk = replace(name_trunk,'var/oldpath', 'var/NewPath')
	WHERE scope = 'binaryfile';
  ...
  updateNameHashLoop: LOOP
    ...
    SET ezdbfile_data_new_name_trunk = replace(ezdbfile_name_trunk,'var/newPath', 'var/oldPath');
